...
- **数据结构**  [🧾例题清单](https://doongz.gitbook.io/notes/algorithm/1-shu-ju-jie-gou)
  ...
  - [字符串](./Algorithm/1-数据结构/4-字符串.md)  `顺序、堆分配、块链存储`  `KMP算法`
  - 树
    ...
    - [树状数组 & 线段树](./Algorithm/1-数据结构/5-4-树状数组.md)  `维护区间信息`  `单点更新O(logn)`  `区间查询O(logn)`  `Hard+`
    - [线段树](./Algorithm/1-数据结构/5-5-线段数.md)
    - [字典树](./Algorithm/1-数据结构/5-6-字典树.md)  `前缀树`  `字符串/字符前缀是否存在`  `dfs`  `Hard`
    - [哈夫曼树「挖坑」](./Algorithm/1-数据结构/5-7-哈夫曼树.md)
    - B树 & B+树「挖坑」
...
- **数学**「填坑中」  [🧾例题清单](https://doongz.gitbook.io/notes/algorithm/5-shu-xue)  `倍增`  `快速幂`  `求余`
  - [计算机算法](./Algorithm/5-数学/1-计算机算法.md)  `num->arr（短除法）`  `arr->num`  `bin_to_dec`  `dec_to_bin`  `向上取整`
  - [矩阵运算](./Algorithm/5-数学/2-矩阵运算.md)  `矩阵快速幂`
  - [经典数学算法](https://doongz.gitbook.io/notes/algorithm/5-shu-xue/jing-dian-shu-xue-suan-fa)  `高斯求和`  `余数(同余定理、加、减、乘)`  `蓄水池抽样算法`  `洗牌算法`  `约瑟夫环`
