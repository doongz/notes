...
这是众多你可以通过page table实现的有意思的事情之一。你**可以向同一个物理地址映射两个虚拟地址**，你可以不将一个虚拟地址映射到物理地址。可以是一对一的映射，一对多映射，多对一映射。XV6至少在1-2个地方用到类似的技巧。这的kernel stack和Guard page就是XV6基于page table使用的有趣技巧的一个例子。
