...
- 高度抽象的接口通常是可移植的，可以在各种各样的存储介质上实现文件和目录
- 向应用程序隐藏复杂性，Linux/Unix提供地址空间的抽象而不是直接访问MMU硬件的权限
- 帮助管理共享资源
- 依赖多个子系统的工具更容易实现，因为所有这些功能都在一个程序里面
- 内核的所有代码都以完整的硬件权限在运行（Supervisor mode）

劣势：

- 大且复杂
- 慢，随着时间的推移，它们倾向于发展成拥有所有的功能。对于各种不同的场景都能支持，或许就不能对某些特定场景进行优化
- 削弱一些复杂的抽象能力
- 可扩展性（Extensibility）不强，实时更改内核比较难
...
- 微内核只提供了**进程抽象和通过IPC进程间通信的方式**
- 文件系统、TCP通信的网络协议栈、虚拟内存系统，fork/exec 在用户空间实现

案例：当文本编辑器VI需要读取一个文件时，需要与文件系统进行交互

1. 它通过IPC会发送一条消息到文件系统进程 
2. 文件系统进程中包含了所有的文件系统代码，它知道文件，目录的信息
3. 文件系统进程需要与磁盘交互，所以它会发送另一个IPC到磁盘驱动程序
4. 磁盘驱动程序再与磁盘硬件进行交互，之后磁盘驱动会返回一个磁盘块给文件系统
5. 文件系统再将VI请求的数据通过IPC返回给VI

L4微内核场景

- iphone中的一个旁路处理器中，使用了L4微内核来隐藏加密密钥
- 微型嵌入式系统中，微内核能够胜出，专门为某种功能设计的计算机

### 3、微内核有什么好处

- 优雅，审美（sense of aesthetic）
- 更小的内核或许会更加的安全
- 需要证明一个操作系统是正确的，没有Bug，如seL4
- 少量代码的程序比巨大的程序更容易被优化
- 运行的更快
- 应用程序的设计限制也更少，应用程序更灵活
- 使得代码更模块化
- 用户空间代码通常会比内核更容易被修改，调整和替换
- 将操作系统的功能移到用户进程可以使得系统更加健壮。这对于驱动来说尤其明显
- 以在微内核上模拟或者运行多个操作系统

挑战：

- 设计微内核，什么才是有用的系统调用的最小集？ 
- 仍然需要开发一些用户空间服务来实现操作系统的其他部分
- 微内核的设计需要进程间通过IPC有大量的通信，IPC能够足够的快
- 将宏内核拆分成不同的服务，运行在用户空间，会影响性能

### 4、L4 micro kernel构成

L4微内核很经典，有15-20个变种，有一些从1980年代开始开发的项目现在还存在

- L4 只有7个系统调用

- 论文发表的时候，L4只有13000行代码

- L4只包含几个非常基础的抽象，Task，线程，地址空间，IPC

  - 一个叫做Task或者地址空间的概念，对应了Uinx内的进程概念。
  - 地址空间，包含了一些内存，地址从0开始。
  - 每个Task可以有多个线程
  - IPC。每一个线程都有一个标识符，线程可以向拥有这个标识符的另一个线程发送几个字节

涉及到的系统调用：

- Threadcreate系统调用，你提供一个地址空间ID并要求创建一个新的线程。如果地址空间或者Task不存在，系统调用会创建一个新的Task。所以这个系统调用即可以创建线程，又可以创建Task。
- Send/Recv IPC系统调用
- Mapping系统调用可以映射内存Page到当前Task或者其他Task的地址空间中
...
- L4会完成线程调度和context switch，来让多个线程共用一个CPU（类似xv6的trap）
- Pager。如果一个进程触发了Page Fault，通过trap走到了内核，内核会将Page Fault转换成IPC消息并发送到指定的Pager Task，并告诉Pager Task是哪个线程的哪个地址触发了Page Fault。Pager Task可以实现基于Page Fault的各种技巧

L4 的设计非常依赖IPC，对于每个系统调用，每个Page Fault，每个设备中断，都会有反复的IPC消息。所以IPC系统需要非常快

### 5、IPC 性能

一个简单，但也慢的设计，基于Unix Pipe

案例：有两个进程，P1和P2，P1想要给P2发送消息。使用send系统调用，传入你想将消息发送到的线程的ID，以及你想发送消息的指针

**一种异步的实现**，P1要完成一次完整的消息发送和接收，有两个buffer，一个用来发送消息，一个用来接收消息

1. P1会先调用send，消息会被先送到内核的【消息buffer】中
2. send返回之后。P1会立即调用recv，recv会等待【接收消息的buffer】出现数据
3. 所以此时，P1会出让CPU（在一个单CPU的系统中，只有当P1出让了CPU，P2才可以运行）
4. 这时，P2才会被调度，之后P2调用recv，从内核的【发送消息buffer】中拷贝消息
5. P2自己再调用send将回复消息追加到【接收消息buffer】
6. 之后P2的send系统调用返回
7. 设在某个时间，或许因为定时器中断触发导致P2出让CPU，这时P1可以恢复运行
8. 内核发现在接收消息buffer有了一条消息，会返回到用户空间的P1进程
...
- 至少一次线程调度和context switching来从P1切换到P2

每一次用户空间和内核空间之间的切换和context switching都很费时，因为每次切换，都需要切换Page Table，进而清空TLB，也就是虚拟内存的查找缓存，这些操作很费时

**同步（Synchronized）的实现**

- 不会丢下消息并等待另一个进程去获取消息
- send会等待消息被接收，并且recv会等待回复消息被发送

过程：

1. 进程P1，发送消息，调用send
2. send并不会拷贝P1的消息到内核的缓存中，P1的send会等待P2调用recv
3. P2要么已经在内核中等待接收消息，要么P1的send就要等P2下一次调用recv
4. 当P1和P2都到达了内核中，P1因为调用send进入内核，P2因为调用recv进入内核
5. 这时才会发生一些事情

快的原因：

- 如果P2已经在recv中，P1在内核中执行send可以直接跳回到P2的用户空间，这种方式也被称为unbuffered。它不需要buffer一部分原因是因为它是同步的
- 当send和recv都在内核中时，内核可以直接将消息从用户空间P1拷贝到用户空间P2
- 如果消息超级小，可以在寄存器中传递，而不需要拷贝，你可以称之为Zero Copy
- 对于非常长的消息，L4可以在一个IPC消息中携带一个Page映射，这个Page会被再次映射到目标Task地址空间
- 如果是个RPC，有request和response，会结合send和recv两个系统调用为一个，减少用户态和内核态的切换

### 6、linux运行在L4微内核上

微内核实际上在一些特殊的应用场合，设备的控制器，车里的点火控制器

微内核项目发起时，是希望取代大的monolithic kernel

- 一种难的方式，重新以微内核的方式，以大量的进程实现所有的内容
- 一种容易的方式，将一个现有的monolithic kernel运行在微内核之上

L4微内核位于底部，一个完整的Linux作为一个巨大的服务运行在用户空间进程中，就像在QEMU上运行XV6

需要对Linux的底层做一些修改，直接对page table 和 cpu寄存器的操作替换成L4的系统调用

- 将Linux内核作为一个L4 Task运行
- 每一个Linux进程又作为一个独立的L4 Task运行
- 运行一个Shell或者其他，会在用户空间创建一个L4 Task
- Linux不会直接修改进程的Page Table，而是会向L4发送正确的IPC让L4来修改进程的Page Table

### 7、L4 Linux性能分析

可以构建类似上一节的架构，花费足够的精力去优化性能，可以获取与原生操作系统相比差不多的性能

论文表明：可以因为其他原因不喜欢微内核，但是不能使用性能作为拒绝微内核的原因
