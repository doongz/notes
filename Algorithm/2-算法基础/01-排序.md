...
def bubble_sort(arr):
    ...
    for i in range(n - 1):  # 外层循环控制排序的轮数，总共需要 n-1 轮
        swapped = False  # 用于优化冒泡排序，标记本轮是否发生交换
        for j in range(n - 1 - i):  # 内层循环负责每轮的相邻元素比较和交换
            if arr[j] > arr[j + 1]:  # 如果相邻元素顺序错误（升序情况下）
                arr[j], arr[j + 1] = arr[j + 1], arr[j]  # 交换它们的位置
                swapped = True  # 标记发生了交换
        if not swapped:  # 如果本轮没有发生交换，说明数组已经有序
            break  # 提前结束排序
    return arr
...
void bubbleSort(vector<int>& arr) {
    ...
    for (int i = 0; i < n - 1; ++i) {  // 外层循环控制排序的轮数
        bool swapped = false;  // 用于优化冒泡排序，标记本轮是否发生交换
        for (int j = 0; j < n - 1 - i; ++j) {  // 内层循环负责每轮的相邻元素比较和交换
            if (arr[j] > arr[j + 1]) {  // 如果相邻元素顺序错误（升序情况下）
