...
- `((state >> num) & 1) == 0` 判断 num 有没有用过，等于 0 没用过，大于 0 用过，`(1 << num) & state) == 0` 也行，但前者更好理解
- `(1 << num) | state` 记录 num 用过了
...
int main() {
    int visited = 0;  // 记录用过的数，初始时为0

    cout << ((1 << 3) & visited) << endl;  // 0 检查 3 有没有被用过，没有
    visited = (1 << 3) | visited;          // 将 3 记录在 visited 这个数中，传递下去
    cout << dec_to_bin(visited) << endl;   // 1000

    cout << ((1 << 3) & visited) << endl;  // 8 检查 3 有没有被用过，用过
    cout << ((1 << 4) & visited) << endl;  // 0 检查 4 有没有被用过，没有
    visited = (1 << 4) | visited;          // 将 4 记录在 visited 这个数中，传递下去
    cout << dec_to_bin(visited) << endl;   // 11000

    cout << ((1 << 3) & visited) << endl;  // 8 检查 3 有没有被用过，用过
    cout << ((1 << 4) & visited) << endl;  // 16 检查 4 有没有被用过，用过
    cout << ((1 << 6) & visited) << endl;  // 0 检查 6 有没有被用过，没有
    visited = (1 << 6) | visited;          // 将 6 记录在 visited 这个数中，传递下去
    cout << dec_to_bin(visited) << endl;   // 1011000

    int pre = visited & (~(1 << 6));  // 查看选择 6 之前的 visited
