...
public:
    ...
    int dfs(int i, int j) {
        ...
        // 结尾两个字母不一样
        int res1 = dfs(i - 1, j - 1)+1;  // 可以将w1[i] 「替换」成与 w2[j]相同的数，两个都消耗
        int res2 = dfs(i - 1, j)+1;      // 将w1[i]「删除」掉，在w1[0,i-1]中找替换成w2[0,j]的方法，并返回操作数
        int res3 = dfs(i, j - 1)+1;      // 在w1的i+1位置上，「插入」一个与w2[j]相同数，w1[i]没有消耗，w2[j]消耗了
...
public:
    ...
    // 返回从word1[0,i] 变到 word2[0,j] 的最小操作数
    int dfs(int i, int j) {
        if (i==-1) return j+1;
        if (j==-1) return i+1;
    ...
    int minDistance(string word1, string word2) {
        W1 = word1;
        W2 = word2;
        mem.resize(W1.size(), vector<int>(W2.size(), -1));
        return dfs(W1.size()-1, W2.size()-1);
    }
};
```
