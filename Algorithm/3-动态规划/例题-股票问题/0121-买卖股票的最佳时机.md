...
- `dp[i][0]`：第 `i` 天结束时**不持有股票**的最大现金（数值越大表示收益越高）。
- `dp[i][1]`：第 `i` 天结束时**持有股票**的最大现金（现金为负表示买入花费，数值越大表示持仓成本越低）。

**状态转移**：

- 不持有股票：要么前一天就不持有，要么当天卖出（前一天持仓的现金 + 当天股价）。
- 持有股票：要么前一天就持有，要么当天买入（仅花当天股价的钱）。

**初始化**：

- 第 0 天不持有股票：`dp[0][0] = 0`（无交易，现金为 0）。
- 第 0 天（第一天）持有股票：`dp[0][1] = -prices[0]`（买入第一天的股票，现金为负）。

**结果**：最终最大利润为 `dp[n-1][0]`（最后一天不持有股票的收益，持有股票无意义）。
