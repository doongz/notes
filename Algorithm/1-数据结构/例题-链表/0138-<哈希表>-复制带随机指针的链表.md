...
public:
    Node* copyRandomList(Node* head) {
        // new->rand:org->rand:new->rand
        unordered_map<Node*, Node*> new2org;
        unordered_map<Node*, Node*> org2new;
        Node* hair = new Node(0);
        Node* newnode = hair;
        while (head) {
            newnode->next = new Node(head->val);
            new2org[newnode->next] = head;
            org2new[head] = newnode->next;
            head = head->next;
            newnode = newnode->next;
        }
        Node* node = hair->next;
        while (node) {
            node->random = org2new[new2org[node]->random];
            node = node->next;
        }
        return hair->next;
    }
