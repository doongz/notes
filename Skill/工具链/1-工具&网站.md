...
```javascript
// 移除知乎渲染后的SVG公式，直接显示LaTeX源码
document.querySelectorAll('.ztext-math svg').forEach(svg => svg.remove());
document.querySelectorAll('.ztext-math').forEach(mathEl => {
  const texCode = mathEl.getAttribute('data-tex');
  if (texCode) mathEl.innerText = `$${texCode}$`;
});
// 阻止后续公式渲染
window.addEventListener('load', () => {
  if (window.katex) window.katex = null;
}, { once: true });
```
